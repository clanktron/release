...
func TestCommitSemverChange(t *testing.T) {
	tests := []struct {
		...
		{"noop with scope", "chore(scope): anything that", noop},
		{"patch with scope", "fix(scope): a patch change", patch},
		{"minor with scope", "feat(scope): a minor change", minor},
		{"major with scope", "feat(scope)!: something something", major},
